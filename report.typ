...
$X1Y1$ is the initial state for rover 1 and $X4Y2$ is the initial state for rover 2. Only the initial states have been marked since those have been considered to be the "stable" states. Other possible choices could have been marking the states representing tiles with charging stations or all the states.

=== Battery

Each $Battery{i}$ plant contains a state for every battery level, representing the rover's battery currently being at that level. The states have been named $L{l}$ where $l$ is the current battery level and ranges from 0 to 6. $L6$ is the initial state since rovers start fully charged. Only $L6$ have been marked since that has been considered to be a "stable" state. Other possible choices could have been marking $L0$ too, or all the non-zero level states, or all the states.
...
The $X2Y2$ states represent the rovers being on the red tile, where the movements are uncontrollable. Hence the following edges have been defined for them:
...
There are also self-edges with event $charge{i}$ to model the charging of the rovers in the states $X1Y1$ and $X4Y2$, limiting the ability to charge the battery only on the corresponding tiles.
...
There's however no way to directly express the "at station $j$" edges since it isn't possible to distinguish at which station the $charge{i}$ event was performed. It's thus necessary to also track in which position the rover is currently at, which can be done by creating 3 copies of the $Position{i}$ plant states, one for every state in the automaton in the figure. Given $S_i$ the initial state in $Position{i}$, ${S_i}I$ will be the new initial state. All states will be marked, since marking is not relevant to this requirement.

More formally, for every state ${S}$ in $Position{i}$ three states have been defined: ${S}I$, ${S}N1$ and ${S}N2$. For every edge with event different than $charge{i}$ between states ${S 1}$ and ${S 2}$ it has been defined an edge between states ${S 1}{M}$ and ${S 2}{M}$ for all $M in {I, N1, N2}$. The following edges have also been defined:
...
- an edge with event $charge{i}$ from $X1Y1 N1$ to $X1Y1 N2$, representing the rightmost edge in the automata above;
- an edge with event $charge{i}$ from $X4Y2 N2$ to $X4Y2 N1$, representing the center edge in the automata above.

This way all the $charge{i}$ edges represent one of the edges in the automata shown in the figure before, while all the other edges correspond to the existing movement edges.
...
The structure of the problem can be exploited to reduce the number of states that have to be manually declared. There are in fact only 2 stations, so they must differ in either their X or Y coordinate. The requirement automata can then only track that coordinate instead of both of them, and that will be enough to distinguish at which charging station the $charge{i}$ is being performed, since there exist only at most one charging station for every value of that coordinate. In this case the charging stations positions differ by both coordinates, so the one with fewer states to track has been chosen, which is Y. The automata can hence be reduced to 9 states: $Y{y}{M}$ for $1 <= y <= 3$ and $M in { I, N1, N2 }$ and the following edges:
...
The initial states are $-3$ and $-1$, because those are differences of the initial coordinates of the rovers. All states have been marked, since marking is irrelevant to this requirement. To satisfy the requirement it's then sufficient to compute the synchronous product of the two automata and remove the state corresponding to the relative position on both axes being 0, which represents the two rovers being on the same tile.
This ends up requiring the user to specify only 44 states.
...
If the introduction of a new uncontrollable event $sametile$ was allowed, an even more compact requirement automaton can be created. This event will never happen, in fact it will be used to have the supervisor remove any state that can perform this event, so it won't create any new execution of the system. The way this is done is by creating an automaton with two states, an initial marked state $Valid$ and another unmarked state $Invalid$, with a single edge with event $sametile$ from $Valid$ to $Invalid$. This way if the $sametile$ event ever happens it will lead to a blocking state and thus the supervisor will have to remove it; moreover since it is uncontrollable it will have to remove any state that can execute it or reach it with uncontrollable events.

Then the two automata for the relative $X$ and $Y$ positions can be kept separate. A self-loop with event $sametile$ have been added to the states $S X$ and $S Y$, this way if the rovers are in the same position, that is the two automata are in the states $S X$ and $S Y$, they will be able to perform the event $sametile$ and reach a blocking state, hence the supervisor will be forced to remove this state.
